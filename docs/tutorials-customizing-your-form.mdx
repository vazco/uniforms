...
import GuestForm4 from './components/GuestForm4.js';
...
Here we've changed the order of the `lastName` and `firstName` field and added a polite question if the people
filling the form would like to share with us their work experience.
...
Instead of using the `ErrorsField` component, we can use the `ErrorField` and just place it directly where want in our form.
