...
function buildGridLabel(size, value, side) {
  return (side === 'label' ? `col-${size}-${value}` : `col-${size}-${(12 - value)}`);
}
export default function buildGrid(grid, side) {
  if (!grid) return '';
  ...
  if (typeof grid === 'number') {
    ...
    return buildGridLabel('sm', grid, side);
  }

  if (typeof grid === 'object') {
    // grid value is an object config
    // eg: { xs: 6, sm: 4, md: 3 }
    return Object.keys(grid).map((value, size) => buildGridLabel(size, value, side));
  }
}
